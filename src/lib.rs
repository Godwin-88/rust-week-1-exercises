...
pub const CURRENT_BLOCK_HEIGHT: u64 = 840000;
...
pub fn unpack_wallet_info(wallet_info: (String, f64)) -> String {
    ...
    format!("Wallet {} has balance: {} BTC", name, balance as i64)
}
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    // Transaction version is first 4 bytes
    if bytes.len() < 4 {
        return Err("Transaction data too short".to_string());
    }

    // Convert first 4 bytes to u32 (little-endian)
